...
# eliminar de una base de datos SQLite para los esquemas v10-v12
# (es decir, para los clientes de captura v3 y v4).
...
encontrar_duplicados_v10_v12 <- function(conexion_sqlite, conexion_postgres){
  ...
  Transecto_especies_invasoras_muestra <- tbl(conexion_sqlite, "Transecto_especies_invasoras_muestra")
  Especie_invasora <- tbl(conexion_sqlite, "Especie_invasora")
  Transecto_huellas_excretas_muestra <- tbl(conexion_sqlite, "Transecto_huellas_excretas_muestra")
  ...
  cantidad_archivos_especies_invasoras_muestra <- Conglomerado_muestra %>%
    ...
    inner_join(Transecto_especies_invasoras_muestra %>%
        select(
          transecto_especias_invasoras_id = id,
          conglomerado_muestra_id
        ), by = "conglomerado_muestra_id") %>%
  ...
  cantidad_archivos_huellas_excretas_muestra <- Conglomerado_muestra %>%
    ...
    inner_join(Transecto_huellas_excretas_muestra %>%
        select(
